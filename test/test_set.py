...
class TestSet:
    ...
    def test_serialize_set(self, n: int):
        ...
        assert orjson.dumps(fs, option=orjson.OPT_SERIALIZE_SET) == orjson.dumps(
            list(fs)
        )
    ...
    def test_roundtrip_set(self, n: int):
        ...
        assert (
            frozenset(orjson.loads(orjson.dumps(fs, option=orjson.OPT_SERIALIZE_SET)))
            == fs
        )
    ...
    def test_roundtrip_set_empty(self):
        ...
        assert (
            orjson.loads(orjson.dumps(frozenset(), option=orjson.OPT_SERIALIZE_SET))
            == []
        )

    def test_nested(self):
        assert (
            orjson.dumps([[{1}, {2}], {3}], option=orjson.OPT_SERIALIZE_SET)
            == b"[[[1],[2]],[3]]"
        )
        fs = frozenset([frozenset([1, 2]), frozenset([3, 4])])
        # order is not guaranteed
        assert (
            orjson.dumps(fs, option=orjson.OPT_SERIALIZE_SET) == b"[[1,2],[3,4]]"
            or orjson.dumps(fs, option=orjson.OPT_SERIALIZE_SET) == b"[[3,4],[1,2]]"
        )
    ...
    def test_no_subclass_passthrough(self):
        ...
        with pytest.raises(orjson.JSONEncodeError):
            orjson.dumps(
                MySet([1, 2, 3]),
                option=orjson.OPT_SERIALIZE_SET | orjson.OPT_PASSTHROUGH_SUBCLASS,
            )
