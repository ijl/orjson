...
pub fn deserialize(
    ...
    #[cfg(feature = "yyjson")]
    {
        if let Some(default) = default {
            let hm = unsafe { py_dict_to_hashmap(default.as_ptr()) };
    ...
    #[cfg(not(feature = "yyjson"))]
    {
        if let Some(default) = default {
            unsafe {
                crate::deserialize::json::DEFAULT_FUNC = py_dict_to_hashmap(default.as_ptr());
            }
            crate::deserialize::json::deserialize_json::<true>(buffer_str)
        } else {
            crate::deserialize::json::deserialize_json::<false>(buffer_str)
        }
    }
}
